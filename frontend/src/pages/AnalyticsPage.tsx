...
function useAnalyticsData() {
  ...
  useEffect(() => { fetchData(page * LIMIT); }, [page, fetchData]);

  const nextPage = () => { if ((page + 1) * LIMIT < total) setPage(p => p + 1); };
  const prevPage = () => { if (page > 0) setPage(p => p - 1); };

  return { data, loading, error, page, total, nextPage, prevPage };
}

function RecordsTable({ data, loading }: { data: FinancialRecord[], loading: boolean }) {
  if (loading) return (
    <tr><td colSpan={7} className="px-6 py-8 text-center text-slate-500 italic">Loading data...</td></tr>
  );
  if (data.length === 0) return (
    <tr><td colSpan={7} className="px-6 py-8 text-center text-slate-500 italic">No records found. Upload a CSV to get started.</td></tr>
  );
  return (
    <>
      {data.map((row) => (
  ...
  return (
    <div className="bg-slate-950 p-4 flex items-center justify-between border-t border-slate-800">
      ...
      <div className="flex gap-2">
        <button onClick={onPrev} disabled={page === 0} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          <ChevronLeft className="h-5 w-5" />
        </button>
        <button onClick={onNext} disabled={(page + 1) * LIMIT >= total} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
